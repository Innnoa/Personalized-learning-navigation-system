...
- 画像页直达资源：在“当前待补强知识点”中点击“查看推荐资源”后，可直接为该薄弱节点生成一版最小资源上下文并跳转到对应资源页
- 路径前后对比区：提交反馈后可查看调整前本轮安排、调整后本轮安排和关键变化
- 反馈快捷录入：首页路径规划和细化图谱局部反馈都支持“已完成并掌握 / 看过但不熟 / 未完成”三种快捷预设，点击后会同时填入完成状态和掌握度，后续仍可手动微调
- 反馈逐步回退：路径规划面板支持按最近一次反馈批次逐步撤销误点或误判，并展示回退前后路径对比
- 演示状态一键恢复：首页健康检查卡片下方提供“恢复演示初始状态”按钮，可一键清空反馈记录、资源行为并恢复默认掌握度，便于重复演示
- 首页路径规划布局：首页主区域当前专注于目标设置、路径结果、学习反馈、回退和路径对比，资源仍从路径项内直接进入单节点资源页
...
- 前端导航：顶部已提供“首页 / 学习图谱 / 推荐资源 / 学习者画像”切换入口；其中“推荐资源”会在存在最近一次资源快照时作为可点击入口出现，便于在多个主页面之间随时回看资源页
...
1. 首页点击“恢复演示初始状态”，确保画像、反馈记录、资源行为和图谱浏览状态都回到统一基线。
2. 进入“学习图谱”页，双击某个可细化节点下钻，点击一个主节点后使用“设为当前学习目标”跳回首页。
3. 在首页查看“本次路径为什么这样安排”，先讲整条路径逻辑，再讲“本轮推荐学习 / 下一轮建议”。
4. 点击“导出本次学习路径”，展示系统可以输出可复用的文本结果。
5. 在学习反馈区直接使用快捷预设按钮提交一轮反馈，观察路径前后对比。
6. 如需演示误判修正，再点击“撤销最近一次反馈”，展示系统支持逐步回退。
7. 从路径项进入“推荐资源”页，展示该节点的配套视频、文本教程、选择原因、对应推荐理由，以及需要时出现的“来自上层”继承提示。
8. 进入“学习者画像”页查看最新学习记录与能力变化，再回到“学习图谱”页，确认图谱仍停留在刚才浏览的层级。
...
- 健康检查卡片是否显示后端状态正常
- 顶部导航是否可以在“首页 / 学习图谱 / 推荐资源 / 学习者画像”之间切换
...
- 在“当前待补强知识点”中点击“查看推荐资源”后，是否能直接跳转到对应节点的单节点资源页
...
- 学习反馈区是否存在“已完成并掌握 / 看过但不熟 / 未完成”三种快捷预设；点击任一按钮后，对应节点的完成状态与掌握度滑块是否会同步更新
