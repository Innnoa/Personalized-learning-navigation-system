...
### [2026-05-12] 第一阶段文档同步为掌握度分段资源建议、画像图表聚合与 practice-check 占位分支

- 变更类型：系统改动
- 变更内容：
  1. README 中的资源建议语义从较宽泛的“配套资源推荐”收紧为“按掌握度阶段组织的资源建议”，并补充“每个路径节点最多返回 4 条资源”的当前约束。
  2. 学习者画像接口说明补充画像图表所需的 analytics 聚合数据，前端文档同步明确画像页会展示掌握度分布与反馈统计图表。
  3. 学习反馈链路文档补充可选的 practice-check 分支，当前仅作为 feedback 后的占位交接入口，用于后续练习检测能力衔接，不表示题库或正式练习能力已完成实现。
- 变更原因：
  第一阶段能力已从“有无资源推荐/画像展示/反馈提交”扩展到更明确的分段推荐、画像图表分析与反馈后续衔接语义。若 README 与架构留痕文档不及时同步，容易在演示或后续开发中把当前阶段能力描述得过宽，甚至误解为题库能力已经落地。
- 影响范围：
  `README.md`、`AIREAD.md`
- 是否影响既有接口/数据库/算法：
  不引入新的数据库结构调整；接口语义说明收紧为“掌握度阶段资源建议”和“画像图表聚合数据”，并为反馈流补充可选的 practice-check 文档分支说明。
- 验证方式：
  1. 检查 `README.md` 的“当前已实现”段落，确认已补充掌握度阶段资源建议、画像图表和 practice-check 占位交接描述。
  2. 检查 `AIREAD.md` 末尾新增记录，确认变更语义与当前第一阶段范围一致，且没有把 question bank / 题库描述成已实现能力。

### [2026-05-12] 补充多角色统一登录与角色分流架构记录

- 变更类型：架构改动
- 变更内容：
  1. 在原有默认演示学习者链路之外，补充“用户身份层”和“学习实体层”的区分：
     - 用户身份层用于承接登录身份、角色类型与登录后路由分流。
     - 学习实体层继续保留既有 learner / student learning profile 语义，用于画像、掌握度、路径规划、反馈与资源推荐。
  2. 当前阶段明确“learners 仍然是学生学习实体”，不会把教师、管理员直接并入路径规划与学习反馈数据模型；教师 / 管理员主要用于进入各自工作台视图。
  3. 前端结构调整为“统一登录入口 + 角色路由分流”：
     - 学生身份进入学生学习系统，继续承接原有知识图谱、路径规划、资源推荐与学习反馈主链路。
     - 教师身份进入教师工作台。
     - 管理员身份进入管理员工作台。
  4. 教师工作台与管理员工作台当前定位为轻量只读演示工作台，主要展示概览、导航和后续可扩展结构，不引入完整注册、细粒度 RBAC 或重型 CRUD 后台能力。
- 变更原因：
  第二阶段需要把系统从“单一学习者演示入口”扩展到“统一登录后的多角色入口”，但毕设当前主价值仍然是学生学习导航主链路。通过把身份层与学习实体层分开，可以在不改动核心 learner 语义的前提下补充教师 / 管理员入口，并避免过早把系统描述成完整教务后台或权限平台。
- 影响范围：
  登录入口语义、前端路由组织、角色首页结构、系统说明文档表述边界。
- 是否影响既有接口/数据库/算法：
  不直接改变路径规划、学习反馈、推荐理由与图谱算法的核心语义；`learner` 相关接口仍主要服务学生学习实体。当前记录的重点是身份分层与前端角色路由结构，而不是引入完整权限系统。
- 验证方式：
  1. 检查 `README.md`，确认已明确写出统一登录入口、角色分流与教师 / 管理员轻量只读工作台定位。
  2. 检查本条记录措辞，确认未把当前阶段描述成“支持注册”“完整 RBAC”或“重型后台 CRUD 系统”。
