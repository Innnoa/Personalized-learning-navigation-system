<template>
  <section class="planner-fragment">
            ...
            <ol class="path-list">
              <li v-for="item in scheduledItems" :key="item.code" class="path-item">
                ...
                >
                  ...
                  <div
                    v-if="getLearningResources(item).length > 0"
                    class="resource-block"
                  >
                    <p class="resource-block-title">配套资源</p>
                    <ul class="resource-list">
                      <li
                        v-for="resource in getLearningResources(item)"
                        :key="`${item.code}-${resource.url}`"
                        class="resource-item"
                      >
                        ...
                        >
                          <li
                            ...
                            :key="`${item.code}-${resource.url}-${tag}`"
    ...
    <template v-if="planResult">
      <article class="result-card resource-recommendation-card planner-extension-card">
          ...
          <div v-else class="resource-sections">
            <section
              v-for="section in resourceRecommendations"
      ...
      <section class="card feedback-panel planner-extension-card">
          <div class="feedback-head">
            <div>
...
function getLearningResources(item) {
  return item?.learningResources || [];
}
